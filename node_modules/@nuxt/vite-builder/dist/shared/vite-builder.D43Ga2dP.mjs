...
import { useNuxt, logger, resolvePath, tryResolveModule, requireModule, useNitro, isIgnored, addVitePlugin } from '@nuxt/kit';
import replace from '@rollup/plugin-replace';
import { resolve, findStaticImports, parseStaticImport, sanitizeFilePath } from 'mlly';
import { parseURL, parseQuery, joinURL, withoutLeadingSlash, withTrailingSlash, withoutBase, withLeadingSlash } from 'ufo';
...
async function buildClient(ctx) {
  ...
  clientConfig.build.rollupOptions = defu(clientConfig.build.rollupOptions, {
    output: {
      chunkFileNames: ctx.nuxt.options.dev ? void 0 : withoutLeadingSlash(join(ctx.nuxt.options.app.buildAssetsDir, "[hash].js")),
...
function virtual(vfs) {
  ...
  return {
    ...
    resolveId(id, importer) {
      ...
      if (resolvedId) {
        return PREFIX$1 + resolvedId;
      }
      if (importer && !isAbsolute(id)) {
        const importerNoPrefix = importer.startsWith(PREFIX$1) ? importer.slice(PREFIX$1.length) : importer;
        const importedDir = dirname(importerNoPrefix);
        const resolved = resolveWithExt(join(importedDir, id));
        if (resolved) {
          return PREFIX$1 + resolved;
        }
      }
      return null;
    },
    load(id) {
      ...
      const idNoPrefix = id.slice(PREFIX$1.length);
...
const VitePublicDirsPlugin = createUnplugin(() => {
  ...
  return {
    ...
    vite: {
      ...
      resolveId: {
        enforce: "post",
        handler(id) {
          if (id === "/__skip_vite" || !id.startsWith("/") || id.startsWith("/@fs")) {
            return;
          }
